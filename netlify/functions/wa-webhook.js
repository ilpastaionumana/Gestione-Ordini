...
// ── Cerca l'ordine il cui ultimo messaggio WhatsApp inviato ha questo wamid (campo waLastMsgId su "orders") ──
// Usato per le spunte di lettura mostrate sulla card ordine, quando il messaggio è partito dal pulsante
// sulla card / invio automatico (non dalla chat in-app, che ha già il suo doc in "wa_messages").
async function queryOrderIdByWamid(wamid, accessToken) {
  ...
  const body = {
    structuredQuery: {
      ...
      where: {
        fieldFilter: {
          field: { fieldPath: "waLastMsgId" },
          op: "EQUAL",
          value: { stringValue: wamid }
        }
      },
      limit: 1
    }
...
// Due possibili origini, cercate in ordine:
//  1) messaggio inviato dalla chat in-app -> doc già esistente in "wa_messages" con ID = wamid;
//  2) messaggio inviato dal pulsante sulla card ordine / invio automatico -> nessun doc in "wa_messages",
//     si cerca invece l'ordine con campo waLastMsgId = wamid e si aggiorna direttamente quello.
async function updateMessageStatus(status, accessToken) {
  ...
  const existingMsg = await getDocument("wa_messages/" + wamid, accessToken);
  if (existingMsg) {
    await patchFields("wa_messages/" + wamid, patch, accessToken);
    return;
  }

  const orderId = await queryOrderIdByWamid(wamid, accessToken);
  if (orderId) {
    await patchFields("orders/" + orderId, patch, accessToken);
  }
}
