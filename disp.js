...
/* ══════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════ */
(function() {
  // Aggancia pgChange: viene chiamata ogni volta che la commessa inserisce una quantità
  function _patchPgChange() {
    if (typeof pgChange !== "function") { setTimeout(_patchPgChange, 300); return; }
    var _orig = pgChange;
    window.pgChange = function(inp) {
      // Prima esegui l'originale (aggiorna formItems)
      _orig(inp);
      // Poi controlla se ci sono prodotti non disponibili in formItems
      _dispCheckFormItems();
    };
  }
  _patchPgChange();

  // Aggancia anche o15AddProduct per la pulsantiera numerica
  function _patchO15AddProduct() {
    if (typeof o15AddProduct !== "function") { setTimeout(_patchO15AddProduct, 300); return; }
    var _orig = o15AddProduct;
    window.o15AddProduct = function(pid) {
      _orig(pid);
      _dispCheckFormItems();
    };
  }
...
// Controlla formItems e gestisce i prodotti non disponibili
function _dispCheckFormItems() {
  var items = window.formItems || [];
  var pending = window._formPendingItems || [];

  // Trova prodotti in formItems che non sono disponibili
  items.forEach(function(it) {
    var stato = dispGetStato(it.productId);
    if (stato === "disponibile") return;
    var exists = pending.find(function(x) { return x.productId === it.productId; });
    if (!exists && parseFloat(it.qty) > 0) {
      pending.push({ productId: it.productId, productName: it.productName, stato: stato });
    }
  });

  // Rimuovi dai pending i prodotti che sono stati rimossi da formItems
  window._formPendingItems = pending.filter(function(x) {
    return items.some(function(it) { return it.productId === x.productId; });
  });

  if (window._formPendingItems.length > 0) {
    window._formSaveBlocked = true;
    dispUpdateSaveButton();
    // Mostra popup solo se non è già visibile
