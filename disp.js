...
/* ══════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════ */
function dispInit() {
  // Aspetta sia Firebase sia che il profilo utente sia caricato
  var fb = window._fb;
...
/* ══════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════ */
// Intercetta i cambiamenti di quantità tramite event listener sul documento
// Funziona indipendentemente da come app.js gestisce gli eventi interni
(function() {
  function _startDOMListener() {
    // Ascolta l'evento "change" su tutto il documento (bubbling)
    // Viene triggerato quando la commessa esce dal campo quantità
    document.addEventListener("change", function(e) {
      var inp = e.target;
      if (!inp.classList.contains("pgrow-qty")) return;
      // Piccolo delay per dare tempo a pgChange di aggiornare il DOM
      setTimeout(_dispCheckFormItems, 50);
    }, true);

    // Ascolta anche "input" per la pulsantiera o15 che potrebbe usare oninput
    ...
    // Intercetta i click sui pulsanti + della pulsantiera
...
// Controlla disponibilità leggendo direttamente dal DOM del form
function _dispCheckFormItems() {
  var pending = window._formPendingItems || [];

  // Leggi tutti i campi quantità visibili nel form ordine
  var qtyInputs = document.querySelectorAll(".pgrow-qty[data-pid]");
  var activePids = []; // pid con quantità > 0
  ...
  qtyInputs.forEach(function(inp) {
    ...
    // Leggi il nome dal DOM (elemento .pgrow-name nella stessa riga del campo qty)
    // La struttura è: span.pgrow-name con testo "[emoji] Nome prodotto"
    var nomeEl = inp.closest(".pgrow-conf-row") ? inp.closest(".pgrow-conf-row").querySelector(".pgrow-name") : null;
    var nomeDOM = nomeEl ? nomeEl.textContent.trim() : "";
    // Rimuovi emoji iniziale e spazio non-breaking (formato: "🍝 Tagliatelle")
    //   è l'&nbsp; usato in app.js come separatore emoji-nome
    ...
    if (!exists) {
      pending.push({ productId: pid, productName: nome, stato: stato });
    }
  });

  // Rimuovi dai pending i prodotti che non hanno più quantità nel form
  window._formPendingItems = pending.filter(function(x) {
    return activePids.indexOf(x.productId) >= 0;
  });

  if (window._formPendingItems.length > 0) {
...
function dispInviaRichiestaAdmin() {
  ...
  var richiesta = {
    ...
    prodotti: pending.map(function(x) {
      // Leggi qty direttamente dal DOM (formItems non è accessibile)
      var qtyInp = document.querySelector(".pgrow-qty[data-pid='" + x.productId + "']");
      var qty = qtyInp ? (parseFloat(qtyInp.value) || "") : "";
      // Leggi l'unità dal DOM se possibile (elemento vicino al campo qty)
      var unitEl = qtyInp ? qtyInp.closest("tr,div[class*='pgrow']") : null;
      var unitSpan = unitEl ? unitEl.querySelector("[data-unit],[class*='unit']") : null;
      var unit = unitSpan ? unitSpan.dataset.unit || unitSpan.textContent.trim() : "";
      return {
...
function dispStartAdminListener() {
  // Solo per admin — verifica ruolo dal profilo direttamente
...
// Avvia il listener admin dopo che Firebase è pronto E il profilo utente è caricato
// IMPORTANTE: window._userProfile è null finché Firebase Auth non completa il login.
// Il valore di default di _profile() è { role:"admin" }, quindi NON bisogna
// chiamare _isAdmin() prima che _userProfile sia valorizzato, altrimenti
// tutti sembrerebbero admin.
(function() {
  function _tryStart() {
    // Aspetta sia Firebase pronto sia che il profilo sia stato caricato (non null)
...
(function() {
  ...
  function _startCommessaListener() {
    // Aspetta che il profilo sia caricato prima di controllare il ruolo
...
/* ══════════════════════════════════════════════════════════════════
   15. HOOK SUL TASTO SALVA — intercetta window._fSave per bloccare se necessario
       In app.js il tasto Salva chiama window._fSave(eid,emode) che a sua volta
       chiama fSave(). Sovrascriviamo window._fSave direttamente, senza aspettare.
   ══════════════════════════════════════════════════════════════════ */
(function() {
  function _patchFSave() {
    // window._fSave viene assegnato da app.js subito al caricamento (riga 2364)
    // Aspettiamo che sia disponibile
