...
window._pendingCountPrecedente = 0; // quanti prodotti pending c'erano al controllo precedente
...
(function() {
  ...
  document.addEventListener("click", function(e) {
    var btn = e.target;
    // Non ri-scatenare il controllo se il click è dentro i nostri popup
    if (btn.closest && (
      btn.closest("#disp-pending-popup") ||
      btn.closest("#disp-risposta-commessa") ||
      btn.closest("#disp-pending-banner")
    )) return;
    if (btn.classList.contains("pgrow-plus") ||
        btn.classList.contains("o15-pcode") ||
...
/* ──────────────────────────────────────────────────────────────────
   Legge i prodotti non disponibili dal DOM del form e aggiorna stato.
   Viene chiamata ogni volta che l'utente modifica il form.
   ────────────────────────────────────────────────────────────────── */
function _dispCheckFormItems() {
  ...
  qtyInputs.forEach(function(inp) {
    ...
    // Leggi il nome dal DOM: span.pgrow-name nella stessa riga .pgrow-conf-row
    // Il testo è "🍝 Nome prodotto" — cerchiamo lo spazio non-breaking ( )
    ...
    var nbspIdx = nomeDOM.indexOf(" "); // spazio non-breaking tra emoji e nome
    if (nbspIdx >= 0) {
      nomeDOM = nomeDOM.substring(nbspIdx + 1).trim();
    } else {
      // fallback: salta il primo "token" (l'emoji) separato da qualsiasi spazio
      var spaceIdx = nomeDOM.search(/\s/);
      if (spaceIdx >= 0) nomeDOM = nomeDOM.substring(spaceIdx + 1).trim();
    }
    var nome = nomeDOM || pid;

    // Deduplicazione per pid
    if (!nuoviPending.find(function(x) { return x.productId === pid; })) {
      nuoviPending.push({ productId: pid, productName: nome, stato: stato });
    }
  });

  var countPrecedente = window._pendingCountPrecedente || 0;
  var countNuovo = nuoviPending.length;
  ...
  if (countNuovo > 0) {
    ...
    var popupEsistente = document.getElementById("disp-pending-popup");
    if (!popupEsistente) {
      // Prima volta: crea e mostra il popup
      window._pendingCountPrecedente = countNuovo;
      dispShowPendingPopup();
    } else if (countNuovo > countPrecedente) {
      // È stato aggiunto un nuovo prodotto non disponibile:
      // rende visibile il popup e aggiorna la lista
      window._pendingCountPrecedente = countNuovo;
      popupEsistente.style.display = "flex";
      dispAggiornaPendingPopup();
      dispUpdateSaveButton();
    } else {
      // Stessa lista o lista più corta (es. ha cambiato qty): aggiorna solo il testo
      window._pendingCountPrecedente = countNuovo;
      dispAggiornaPendingPopup();
      dispUpdateSaveButton();
    }

    // Invia la richiesta al laboratorio solo una volta per ordine
    if (!window._richiestaInviata) {
      window._richiestaInviata = true;
      dispInviaRichiestaAdmin();
    }
  } else {
    // Nessun prodotto non disponibile nell'ordine
    window._formSaveBlocked = false;
    window._richiestaInviata = false;
    window._pendingCountPrecedente = 0;
    dispClearPendingUI();
    dispUpdateSaveButton();
  }
}
...
/* Costruisce il corpo HTML con la lista dei prodotti in attesa */
function _dispBuildPendingBody(pending) {
  ...
  if (nd.length) {
    bodyHTML += "<div style='font-size:13px;font-weight:700;color:#c0392b;margin-bottom:6px'>" +
      "🔴 Non disponibili — il laboratorio è stato avvisato:</div>" +
      "<ul style='margin:0 0 10px 16px;padding:0'>";
    nd.forEach(function(x) {
      bodyHTML += "<li style='font-size:13px;margin-bottom:3px'>" + x.productName + "</li>";
    });
    bodyHTML += "</ul>";
  }
  if (dc.length) {
    bodyHTML += "<div style='font-size:13px;font-weight:700;color:#e65100;margin-bottom:6px'>" +
      "🕐 Da confermare — attendi la risposta del laboratorio:</div>" +
      "<ul style='margin:0 0 10px 16px;padding:0'>";
...
/* Aggiorna solo il testo del popup già esistente, senza rimuoverlo */
function dispAggiornaPendingPopup() {
  var bodyDiv = document.querySelector("#disp-pending-sheet .disp-pending-body");
  if (bodyDiv) {
    bodyDiv.innerHTML = _dispBuildPendingBody(window._formPendingItems || []);
  }
}

/* Crea il popup dal nulla e lo inserisce nel DOM */
function dispShowPendingPopup() {
  var pending = window._formPendingItems;
  if (!pending || !pending.length) return;

  // Se esiste già, lo rende visibile e aggiorna il contenuto
  ...
  popup.style.cssText = "position:fixed;inset:0;z-index:8000;display:flex;align-items:flex-end;" +
    "justify-content:center;background:rgba(0,0,0,0.3);touch-action:none;";
  ...
  popup.innerHTML =
    ...
    "<div class='disp-pending-body'>" + _dispBuildPendingBody(pending) + "</div>" +
    "<div style='font-size:12px;color:#7a5c45;margin-top:8px;margin-bottom:16px'>" +
    "Il laboratorio è stato avvisato. Puoi continuare a compilare l'ordine " +
    "ma non salvarlo finché non arriva la risposta.</div>" +
    "<button id='disp-pending-close-btn' style='width:100%;padding:13px;background:#f5ede6;color:#C4622D;" +
    "border:none;border-radius:10px;font-size:15px;font-weight:700;cursor:pointer;" +
    "font-family:Lato,sans-serif;-webkit-tap-highlight-color:transparent;" +
    "touch-action:manipulation'>Chiudi e continua</button>" +
    "</div>";

  document.body.appendChild(popup);

  // Bottone chiudi: nasconde il popup.
  // Usiamo touchend per mobile (reagisce al rilascio del dito, non a ulteriori eventi)
  // e click come fallback per desktop.
  var closeBtn = document.getElementById("disp-pending-close-btn");
  function _chiudiPopup(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
    popup.style.display = "none";
  }
  closeBtn.addEventListener("touchend", _chiudiPopup, { capture: true, passive: false });
  closeBtn.addEventListener("click", _chiudiPopup, { capture: true });
...
/* ══════════════════════════════════════════════════════════════════
   8. BLOCCO TASTO SALVA — doppio presidio
   ══════════════════════════════════════════════════════════════════ */
function dispUpdateSaveButton() {
  // Cerca il bottone salva in più modi per essere sicuri di trovarlo
  var saveBtn = document.getElementById("f-save-btn");
  if (!saveBtn) {
    var btns = document.querySelectorAll("#modal-sheet button, #modal-sheet .btn-p");
...
/* Secondo presidio sul click del Salva (capture phase, prima di app.js) */
(function() {
  document.addEventListener("click", function(e) {
    if (!window._formSaveBlocked) return;
    if (!window._formPendingItems || !window._formPendingItems.length) return;
    var btn = e.target;
    var isSaveBtn = (btn.id === "f-save-btn") ||
      (btn.textContent && btn.textContent.trim().startsWith("Salva") &&
       btn.closest && btn.closest("#modal-sheet"));
    if (!isSaveBtn) return;
    e.preventDefault();
    e.stopImmediatePropagation();
    ...
    // Rimostra il popup se era nascosto
    var p = document.getElementById("disp-pending-popup");
    if (p) { p.style.display = "flex"; }
    else { dispShowPendingPopup(); }
  }, true);
...
function dispInviaRichiestaAdmin() {
  ...
  var richiesta = {
    ...
    prodotti: pending.map(function(x) {
      ...
      var unitRow = qtyInp ? qtyInp.closest(".pgrow-conf-row") : null;
      var unitBtn = unitRow ? unitRow.querySelector(".pgrow-unit-mista") : null;
...
function dispRenderPopupAdmin(pv) {
  ...
  // Deduplicazione prodotti per richiestaId + nome
  var visti = {};
  var prodotti = [];
...
function dispRispondiSingolo(richiestaId, productName, esito, pv) {
  ...
  aggiornata.prodotti = richiesta.prodotti.map(function(p) {
    ...
    return Object.assign({}, p, { esito: "confermato" });
  });
...
function dispRiceviRisposta(richiesta) {
  ...
  window._pendingCountPrecedente = (window._formPendingItems || []).length;
...
function dispMostraRispostaCommessa(prodotti) {
  ...
  if (confermati.length) {
    bodyHTML += "<div style='margin-bottom:10px'>" +
      "<div style='font-size:13px;font-weight:700;color:#3a7a2a;margin-bottom:6px'>✅ Disponibile — puoi procedere:</div>";
  ...
  ov.innerHTML =
    ...
    "style='width:100%;padding:13px;background:var(--t);color:#fff;border:none;border-radius:10px;" +
    "font-size:15px;font-weight:700;cursor:pointer;font-family:Lato,sans-serif;margin-top:6px;" +
    "-webkit-tap-highlight-color:transparent;touch-action:manipulation'>Ho capito</button>" +
    "</div>";
  document.body.appendChild(ov);

  var okBtn = document.getElementById("disp-risposta-ok-btn");
  function _chiudiRisposta(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
  ...
  okBtn.addEventListener("touchend", _chiudiRisposta, { capture: true, passive: false });
  okBtn.addEventListener("click", _chiudiRisposta, { capture: true });
}
