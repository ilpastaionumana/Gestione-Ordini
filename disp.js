...
(function() {
  function _startDOMListener() {
    ...
    document.addEventListener("click", function(e) {
      var btn = e.target;
      // Non scatenare il controllo se il click è sui pulsanti del popup di disponibilità
      if (btn.id === "disp-pending-close-btn") return;
      if (btn.closest && btn.closest("#disp-pending-popup")) return;
      if (btn.id === "disp-risposta-ok-btn") return;
      if (btn.closest && btn.closest("#disp-risposta-commessa")) return;
...
function _dispCheckFormItems() {
  ...
  if (nuoviPending.length > 0) {
    ...
    // Mostra il popup SOLO se non è già nel DOM (visibile o nascosto).
    // Se il popup esiste già (magari solo nascosto dopo "Chiudi e continua"),
    // aggiorna solo il testo interno senza ricreare il popup —
    // questo evita che ricompaia subito dopo essere stato chiuso.
...
/* Costruisce l'HTML del corpo del popup (lista prodotti in attesa) */
...
/* Aggiorna il testo del popup già esistente senza rimuoverlo/ricrearlo */
function dispAggiornaPendingPopup() {
  var sheet = document.getElementById("disp-pending-sheet");
  if (!sheet) return;
  var bodyDiv = sheet.querySelector(".disp-pending-body");
  if (!bodyDiv) return;
...
function dispShowPendingPopup() {
  ...
  // Se il popup esiste già, aggiorna solo il contenuto e assicurati che sia visibile
  ...
  var bodyHTML = _dispBuildPendingBody(pending);
  ...
  popup.innerHTML =
    ...
    "<div class='disp-pending-body'>" + bodyHTML + "</div>" +
    ...
    "font-family:Lato,sans-serif;-webkit-tap-highlight-color:transparent'>Chiudi e continua</button>" +
    "</div>";
  ...
  }, true); // capture phase per intercettare prima di qualsiasi altro listener
...
/* Intercetta il click sul bottone Salva in capture phase come ulteriore sicurezza */
(function() {
  document.addEventListener("click", function(e) {
    var btn = e.target;
    // Controlla se è il pulsante salva
    var isSaveBtn = (btn.id === "f-save-btn") ||
      (btn.textContent && btn.textContent.trim().startsWith("Salva") &&
       btn.closest && btn.closest("#modal-sheet"));
    if (!isSaveBtn) return;
    if (window._formSaveBlocked && window._formPendingItems && window._formPendingItems.length > 0) {
      e.preventDefault();
      e.stopImmediatePropagation();
      if (typeof showToast === "function") showToast("⚠️ Attendi la conferma del laboratorio prima di salvare");
      // Rimostra il popup se era stato nascosto
      var p = document.getElementById("disp-pending-popup");
      if (p) { p.style.display = "flex"; }
      else { dispShowPendingPopup(); }
    }
  }, true); // capture phase — intercetta prima di qualsiasi handler di app.js
})();
...
function dispMostraRispostaCommessa(prodotti) {
  ...
  ov.innerHTML =
    ...
    "font-size:14px;font-weight:700;cursor:pointer;font-family:Lato,sans-serif;margin-top:6px;" +
    "-webkit-tap-highlight-color:transparent'>Ho capito</button>" +
    "</div>";
  document.body.appendChild(ov);
  // Assegna il click dopo aver aggiunto al DOM — evita problemi con onclick inline
  document.getElementById("disp-risposta-ok-btn").addEventListener("click", function(e) {
    e.preventDefault();
    e.stopPropagation();
    e.stopImmediatePropagation();
