...
function dispInviaRichiestaAdmin(lista) {
  ...
  window._dispMieRichieste[rid] = richiesta;
...
function _dispRenderAdmin(pv) {
  ...
  // Un prodotto compare UNA sola volta, anche se è in più richieste dello stesso PV
  // (es. richieste rimaste aperte da ordini precedenti). La quantità mostrata è la più recente.
  var visti={}, prodotti=[];
  tutte.forEach(function(r){ (r.prodotti||[]).forEach(function(p){
    var k=_dispChiaveProd(p);
    if (!visti[k]) { visti[k]=true; prodotti.push(Object.assign({},p,{_key:k})); }
  }); });
  ...
  if (prodotti.length === 1) {
    ...
    body =
      ...
      "<button onclick=\"_dispRispondi('"+pv+"','confermato')\" " +
      ...
      "<button onclick=\"_dispRispondi('"+pv+"','non_disponibile')\" " +
    ...
    prodotti.forEach(function(p,i){
      ...
      body += "<label style='display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #f0e4d4;cursor:pointer'>" +
        "<input type='checkbox' id='dchk-"+i+"' data-key='"+String(p._key).replace(/'/g,"&#39;")+"' style='width:18px;height:18px;accent-color:#3a7a2a'>" +
...
/* ══════════════════════════════════════════════════════════════════
   11. RISPOSTE ADMIN
   ══════════════════════════════════════════════════════════════════ */
// Chiave che identifica un prodotto nelle richieste (ID se c'è, altrimenti il nome)
function _dispChiaveProd(p) {
  return p.productId ? "id:" + p.productId : "nome:" + String(p.productName || "").toLowerCase();
}

// Invia la risposta a TUTTE le richieste aperte di quel punto vendita.
// esiti = { chiaveProdotto: "confermato" | "non_disponibile" }
function _dispInviaRisposte(pv, esiti) {
  Object.keys(window._richiesteInAttesa || {}).forEach(function(rid){
    var r = window._richiesteInAttesa[rid];
    if (!r || r.pv !== pv) return;
    var ag = Object.assign({}, r, { stato: "risposta_inviata" });
    ag.prodotti = (r.prodotti || []).map(function(p){
      return Object.assign({}, p, { esito: esiti[_dispChiaveProd(p)] || "non_disponibile" });
    });
...
// Popup con un solo prodotto: pulsanti Confermo / Non disponibile
function _dispRispondi(pv, esito) {
  var esiti = {};
  Object.values(window._richiesteInAttesa || {}).forEach(function(r){
    if (r.pv !== pv) return;
    (r.prodotti || []).forEach(function(p){ esiti[_dispChiaveProd(p)] = esito; });
  });
  _dispInviaRisposte(pv, esiti);
  if (typeof showToast==="function") showToast(esito==="confermato"?"✅ Conferma inviata a "+pv:"❌ Non disponibile inviato a "+pv);
}

// Popup con più prodotti: caselle da spuntare
function _dispRispondiMulti(pv) {
  ...
  popup.querySelectorAll("input[type='checkbox']").forEach(function(chk){
    esiti[chk.dataset.key] = chk.checked ? "confermato" : "non_disponibile";
  });
  _dispInviaRisposte(pv, esiti);
...
function _dispResetForm() {
  // Se la commessa chiude/annulla l'ordine prima della risposta del laboratorio,
  // le sue richieste vengono segnate "annullata" e spariscono dal popup del laboratorio
  try {
    Object.keys(window._dispMieRichieste || {}).forEach(function(rid){
      var r = window._dispMieRichieste[rid];
      if (r && typeof r === "object" && window.fbSaveDoc) {
        window.fbSaveDoc("richieste_conferma", rid, Object.assign({}, r, { stato: "annullata", annullataAt: Date.now() }));
      }
    });
  } catch (e) {}
  window._formPendingItems  = [];
