...
function renderProducts(){
  ...
  var visibleProds=prQ?products.filter(function(p){return (p.name||"").toLowerCase().indexOf(prQ)>-1||(p.shortName||"").toLowerCase().indexOf(prQ)>-1;}):products;
...
function o15BuildGrid(filterText){
  ...
  for(var pgIdx=0; pgIdx<4; pgIdx++){
    ...
    for(var i=1;i<=50;i++){
      ...
      if(group&&q){
        var matches=group.some(function(p){return p.name.toLowerCase().indexOf(q)>-1||(p.shortName||"").toLowerCase().indexOf(q)>-1;});
    ...
    "<div class='o15-bottom-panel'>"+
      "<div class='o15-bottom-hd'>"+
        "<div class='o15-search-box'>"+
          "<span>🔎</span>"+
          "<input id='o15-search-input' inputmode='none' placeholder='Cerca prodotto per nome o sigla…' style='background:transparent;border:none;outline:none;color:inherit;font-size:12px;flex:1' oninput='o15FilterSearch(this.value)' onfocus='o15ShowQwerty()'>"+
...
function buildProdGrid(filterQ){
  ...
  const allGroups=categories.map(c=>({
    cat:c,
    prods:products.filter(p=>p.catId===c.id&&(!q||p.name.toLowerCase().includes(q)||(p.shortName||"").toLowerCase().includes(q)))
  })).concat([{
    cat:{id:"__none__",name:"Senza categoria",icon:"📦"},
    prods:products.filter(p=>(!p.catId||!categories.find(c=>c.id===p.catId))&&(!q||p.name.toLowerCase().includes(q)||(p.shortName||"").toLowerCase().includes(q)))
  }]);
