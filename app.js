...
async function sendWAapiWithFeedback(orderId,stato){
  ...
  if(result.ok){
    ...
    orders=orders.map(x=>x.id===orderId?{...x,waLastSent:Date.now(),waLastStato:stato}:x);
...
function _fmtItemHTML(it, idx, oid, doneSet){
  ...
  const name="<strong style='font-size:16px;white-space:nowrap'>"+it.productName+"</strong>";
...
function orderCardHTML(o,showDate){
  ...
  if((o.stato==="preparato"||o.stato==="pronto")&&!o.importo){
    html+=
...
function renderProducts(){
  ...
  const groups=categories.map(function(c){
    return {cat:c,prods:products.filter(function(p){return p.catId===c.id;})};
  });
  const noCat=products.filter(function(p){return !p.catId||!categories.find(function(c){return c.id===p.catId;});});
...
// ---- pulsantiera: costruzione griglia 2 pagine × 50 codici ----
    ...
    for(var i=1;i<=50;i++){
      ...
      if(!group||group.length===0){
        ...
        html+="<div class='o15-pcode"+(isGroup?" group":"")+(n>0?" selected":"")+"' id='o15-btn-"+code+"' onclick='o15TapCode("+code+")'>"+
          ...
          "<span class='o15-pname'>"+_o15Abbr(display.name)+"</span>"+
...
function o15OpenVariantPicker(code,group){
  ...
  document.getElementById("o15-vp-grid").innerHTML=group.map(function(p){
    ...
    return "<div class='o15-vp-btn"+(sel?" selected":"")+"' onclick=\"o15AddProduct('"+p.id+"')\">"+
      "<span class='o15-vp-name'>"+p.name+"</span>"+
  ...
  root.innerHTML=
    ...
    "<div class='o15-bottom-panel'>"+
      "<div class='o15-bottom-hd'>"+
        ...
        "</div>"+
        "<div class='o15-page-dots' id='o15-page-dots'><div class='d on' onclick='o15GoPage(0)'></div><div class='d' onclick='o15GoPage(1)'></div></div>"+
  ...
  (function(){
    ...
    if(pager){
      ...
      pager.addEventListener("touchend",function(e){
        ...
        if(dx<-40&&_o15CurrentPage===0)o15GoPage(1);
        if(dx>40&&_o15CurrentPage===1)o15GoPage(0);
        touchX=null;
      });
      var mx=null;
...
function _pgConfRowHTML(pid,i,qty,unit,p,isFirst,for15){
  ...
  const nameSpan=isFirst
    ? "<span class='pgrow-name'>"+p.image+"&nbsp;"+p.name+"</span>"
    : "<span class='pgrow-name' style='visibility:hidden;pointer-events:none'>"+p.image+"&nbsp;"+p.name+"</span>";
  ...
  function buildDetailDOM(o){
    ...
    (o.items||[]).forEach(function(it,idx){
      ...
      if(nConf>1&&it.confezioni){
        ...
        innerWrap.style.cssText="display:flex;align-items:center;flex:1;gap:8px;min-width:0";
        if(it.unit==="pz"){
          innerWrap.innerHTML="<span style='font-size:15px;font-weight:700;"+dec+"flex:1'>"+qtyFmt+" "+it.productName+"</span>"+doneIcon;
          ...
          innerWrap.innerHTML="<span style='font-size:15px;font-weight:700;"+dec+"flex:1'>"+it.productName+"</span>"+mistaStr+doneIcon;
        } else {
          innerWrap.innerHTML=
            "<span style='font-size:15px;font-weight:700;"+dec+"flex:1'>"+it.productName+"</span>"+
...
function openProdForm(id){
  ...
  showModal(
    ...
    "<input class='inp' id='pf-code' type='number' inputmode='numeric' min='1' max='100' placeholder='da 1 a 100' value='"+(p&&p.code?p.code:"")+"'>"+
...
function pfSave(existId){
  ...
  if(codeRaw){
    ...
    if(!code||code<1||code>100)return alert("Il codice deve essere un numero tra 1 e 100 (oppure lascialo vuoto).");
  }
  if(!name)return alert("Inserisci il nome del prodotto");
  const prod={id:existId||uid(),name,unit,image:pfImg,catId:catId||"",code:code};
